...
class Farm:
    ...
    def add_animal(self, animal: [Animal]) -> None:
        ...
        print(f'현재 농장 {self._farm_name}있는 동물들은 {animals}이고 총 {self._animal_number}마리 입니다.')


    def feed_all(self, food: str) -> None:
        print(f'-------동물들에게 {food}주는 중---------')
        for i in self._animal:
            i.feed()
            # i._hp += 50
            # if i._hp > 100:
    ...
    def show_all(self):
        print(f'------{self._farm_name}의 동물들은 총 {self._animal_number}마리이며, 정보는 다음과 같습니다.------')
        for i in self._animal:
